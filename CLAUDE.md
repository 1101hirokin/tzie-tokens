...
- `packages/js/` - Published JS/TS package (`@tzie/tokens`)
- `packages/json/` - Published JSON package (`@tzie/tokens-json`)
- `packages/ios/` - SwiftPM package
- `packages/compose/` - Kotlin/Compose package
...
- Main entry: `packages/js/dist/js/tokens.js` (ESM)
- TypeScript definitions: `packages/js/dist/js/tokens.d.ts`
- Theme entry: `packages/js/dist/js/themes.js` with `themes["standard-light"]` access
- CSS import: `@tzie/tokens/tokens.css`

### iOS/SwiftPM
- `packages/ios/Package.swift` defines the SwiftPM package
- Targets `packages/ios/dist/ios/DesignTokens.swift` as source
- Platforms: iOS 14+, macOS 12+

### Kotlin/Compose
- Gradle configuration in `packages/compose/build.gradle.kts`
- Sources `packages/compose/dist/compose/Tokens.kt`
...
- Theme sources live in `src/theme/` but `src/` is not published; `npm run build:themes` builds `dist/js/themes.{js,d.ts}` and copies JSON to `dist/js/theme` and `dist/json/themes`
- `npm run copy-packages` copies `dist/js` and `dist/css` into `packages/js`, plus `dist/compose` into `packages/compose` and `dist/ios` into `packages/ios`
- Prefix all Tzie-specific TypeScript types with `Tzie` (e.g., `TzieTheme`)
